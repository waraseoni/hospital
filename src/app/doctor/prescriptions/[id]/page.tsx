...
import type { Prescription, MedicineItem, Patient } from "@/types/database";
...
function PrescriptionEditForm() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setNotes(rx.notes || "");
      setMedicines(rx.medicines?.length ? rx.medicines : [{ name: "", dosage: "", frequency: "", duration: "", instructions: "" }]);

      const { data: pat } = await supabase.from("patients").select("*").eq("id", rx.patient_id).single();
      setPatient(pat);
      setLoading(false);
    }
    load();
  ...
  if (notFound) return <div className="text-destructive">Prescription not found.</div>;

  return (
    <div className="max-w-2xl">
      <h1 className="text-2xl font-bold mb-2">Edit Prescription</h1>
      {patient && (
