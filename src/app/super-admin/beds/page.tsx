...
export default function SuperAdminBedsPage() {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(t("beds.deleteFailed"));
  ...
  return (
    <div>
      <PageHeader title={t("beds.title")} actions={<Button onClick={openCreate}><Plus size={16} className="mr-1" />{t("beds.addBed")}</Button>} />
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="mb-6 rounded-xl border border-border bg-card p-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div><label className="mb-1 block text-sm font-medium">{t("beds.wardName")}</label><Input value={form.ward_name} onChange={(e) => setForm({ ...form, ward_name: e.target.value })} required /></div>
            <div><label className="mb-1 block text-sm font-medium">{t("beds.bedNumber")}</label><Input value={form.bed_number} onChange={(e) => setForm({ ...form, bed_number: e.target.value })} required /></div>
            <div><label className="mb-1 block text-sm font-medium">{t("beds.bedType")}</label><Select value={form.bed_type} onChange={(e) => setForm({ ...form, bed_type: e.target.value as Bed["bed_type"] })} options={[{ value: "general", label: t("beds.general") }, { value: "semi_private", label: t("beds.semiPrivate") }, { value: "private", label: t("beds.private") }, { value: "icu", label: t("beds.icu") }, { value: "emergency", label: t("beds.emergency") }]} /></div>
            <div><label className="mb-1 block text-sm font-medium">{t("beds.dailyRate")}</label><Input type="number" min={0} value={form.daily_rate} onChange={(e) => setForm({ ...form, daily_rate: Number(e.target.value) })} required /></div>
          </div>
      ...
      {loading ? <Skeleton lines={5} /> : beds.length === 0 ? (
        <EmptyState title={t("beds.noBeds")} description={t("ui.noData")} action={<Button onClick={openCreate}><Plus size={14} className="mr-1" />{t("beds.addBed")}</Button>} />
      ) : (
