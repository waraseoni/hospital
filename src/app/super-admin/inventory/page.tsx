...
export default function SuperAdminInventoryPage() {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(t("inventory.deleteFailed"));
  ...
  return (
    <div>
      <PageHeader title={t("inventory.title")} actions={<Button onClick={() => { resetForm(); setShowForm(true); }}><Plus size={16} className="mr-1" />{t("inventory.addItem")}</Button>} />
      ...
      {loading ? <Skeleton lines={5} /> : items.length === 0 ? (
        <EmptyState title={t("inventory.noItems")} description={t("ui.noData")} action={<Button onClick={() => { resetForm(); setShowForm(true); }}><Plus size={14} className="mr-1" />{t("inventory.addItem")}</Button>} />
      ) : (
      ...
      <Modal open={!!deleteId} onOpenChange={() => setDeleteId(null)} title={t("inventory.deleteConfirm")} footer={
        ...
        <p>{t("inventory.deleteConfirmMsg")}</p>
      </Modal>
    </div>
  );
}
