...
import { LayoutDashboard, Folder, Users2, ClipboardList, AlertTriangle, Pill, ClipboardCheck, FileStack, SprayCan } from "lucide-react";
...
const navItems: NavItem[] = [
  ...
  { href: "/staff/housekeeping", labelKey: "nav.housekeeping", icon: SprayCan },
...
export default function StaffLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      ...
      if (!data || effectiveRoleOf(data.role, imp) !== "staff") { router.push("/login"); return; }
      setProfile(data);
    }
    loadProfile();
  }, [router]);
